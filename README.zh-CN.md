...
- 日剧不需要任何配置就能上手：应用内置 8 部手写精选日剧，每部都写清楚它适合学什么，例如职场敬语或日常会话。
- 想看更多日剧，可以另外配置 TMDB token，用来获取本季与上季（クール）的剧集一览、日剧全量搜索、日语简介、电视台，以及日本地区的正规播放链接。
...
没有 token 也能正常学：内置精选清单、资源搜索、字幕获取、播放和整条学习闭环都不经过 TMDB。只有当你还想要本季与上季（クール）的剧集一览、日剧全量搜索、日语简介、电视台和日本地区正规播放链接时，才需要配置 token。

注册 TMDB 账号后打开 [TMDB API 设置页](https://www.themoviedb.org/settings/api)，复制 **API Read Access Token**，也就是长的那一个。短的「API Key」不能用，因为 tanku Anime 使用 Bearer 请求头认证。把 token 填入 **設定 / Settings** 即可。

与其他密钥一样，它只储存在本机 SQLite 数据库；请勿提交密钥或 `server/data/` 目录。
...
- 日剧作品数据来自 [TMDB](https://www.themoviedb.org/)。本产品使用 TMDB API，但未获得 TMDB 的认可或认证（This product uses the TMDB API but is not endorsed or certified by TMDB.）。
...
日剧模式是最新加入的功能。不需要 token 的部分——内置精选清单、日剧资源搜索、Jimaku 日剧字幕——已对真实服务确认可用；依赖 TMDB 的季度（クール）一览、搜索和详情页则尚未用真实 token 实跑。路线图见 [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md)。
