...
早期段階ですが、作者自身が日常的に使っています。学習の一連の流れは macOS で実メディアを使って確認済みです。Windows は互換修正、クロスプラットフォーム CI、新規導入と起動の確認がありますが、実機でのメディア再生は未確認です。ロードマップは [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md) を参照してください。
