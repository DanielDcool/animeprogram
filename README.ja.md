...
- ドラマは設定なしで始められます。学習向けに手書きした厳選 8 作品を同梱しており、職場の敬語、日常会話など「どこが学習に効くか」を一言ずつ添えています。
- ドラマをもっと広く見たい場合は、任意で TMDB のトークンを設定できます。今期・前期のクール一覧、ドラマの全文検索、日本語のあらすじ、放送局、日本国内の配信リンクが加わります。
...
トークンがなくても学習は一通り成立します。同梱の厳選リスト、リソース検索、字幕の取得、再生、学習の流れはすべて TMDB を経由しません。今期・前期のクール一覧、ドラマの全文検索、日本語のあらすじ、放送局、日本国内の配信リンクも使いたい場合にだけ設定してください。

TMDB のアカウントを作成し、[TMDB の API 設定ページ](https://www.themoviedb.org/settings/api)を開いて、**API Read Access Token**（長い方）をコピーします。短い「API キー」では動作しません。tanku Anime は Bearer ヘッダーで認証するためです。コピーしたトークンは **設定 / Settings** に貼り付けます。

他のキーと同じく、ローカルの SQLite データベースだけに保存されます。キーや `server/data/` ディレクトリをコミットしないでください。
...
- ドラマの作品データは [TMDB](https://www.themoviedb.org/) を利用しています。本製品は TMDB の API を利用していますが、TMDB による推奨・認証を受けたものではありません（This product uses the TMDB API but is not endorsed or certified by TMDB.）。
...
ドラマモードは最も新しい機能です。トークンを必要としない部分——同梱の厳選リスト、ドラマのリソース検索、Jimaku のドラマ字幕——は実際のサービスで確認済みですが、TMDB を使うクール一覧、検索、詳細ページは実トークンでの動作確認がまだ済んでいません。ロードマップは [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md) を参照してください。
