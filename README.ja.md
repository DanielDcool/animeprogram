...
- 単語と文を保存し、詳細ページで保存済みの説明、既存の AI 講解キャッシュ、時間付きの出典リンクを確認してから、Anki 互換の TSV を書き出せます。
- 任意で Jimaku の字幕照合を利用し、magnet リンクを自分のローカルダウンローダーへ渡せます。ダウンローダー RPC、動画転送、リモートのメディア管理は実装していません。

## クイックスタート

**必要環境：** Node.js 22.x、および `PATH` から実行できる `ffmpeg` と `ffprobe`。アプリ全体と実メディアの流れは macOS で手動確認済みです。Windows CI では依存関係の導入、テスト、ビルド、起動、SQLite 初期化、ディレクトリ監視、Web ページ、ヘルスエンドポイントを確認します。Windows 実機でのメディア再生は最終確認が残っています。
...
プロジェクトは公開済みで、クロスプラットフォーム CI、コントリビューション方針、セキュリティ方針があります。macOS は手動確認済みです。Windows には互換修正と自動テストを追加しましたが、実機でのメディア再生確認が残っています。広く紹介する前に、著作権上安全なスクリーンショットまたは短いデモも必要です。現在のロードマップは [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md) を参照してください。
